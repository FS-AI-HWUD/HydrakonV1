...
class ROS2Publisher(Node):
    ...
    def __init__(self):
        ...
        self.current_ami_state = "DEFAULT"
        ...
        try:
            ...
            if ami_state and ami_state != self.current_ami_state:
                ...
                if ami_state == "ACCELERATION":
                    ...
                    if self.current_ami_state not in ["DEFAULT", ami_state]:
...
📡 Mission Status Published to /hydrakon_can/is_mission_completed: {'TRUE ✅' if lap_stats.get('target_reached') else 'FALSE 🔄'}"""
    ...
    def setup_controller(self, target_laps=None):
        ...
        try:
            ...
            print(f"📡 NEW: Driving flag publishing enabled - status to /hydrakon_can/driving_flag (Bool)")
            print(f"📡 NEW: AMI state monitoring enabled - automatic configuration based on /hydrakon_can/state_str")
            print(f"📡 NEW: DRIVING state monitoring enabled - automatic driving flag control")
    ...
    def control_loop(self):
        """Main control loop for vehicle with target lap checking, mission completion publishing, and dynamic configuration"""
        print("Starting configurable cone following control loop with AMI state monitoring and driving flag control...")
        ...
        print(f"📡 Driving Flag: Bool to /hydrakon_can/driving_flag (TRUE when DRIVING:TRUE detected)")
        print(f"📡 AMI State Monitoring: Dynamic configuration based on /hydrakon_can/state_str")
        print(f"📡 DRIVING State Monitoring: Automatic driving flag control based on DRIVING field")
        print(f"📡 ROS2 Subscribing: Cone detections from /zed2i/detections_data")
        print(f"🔄 Waiting for AMI state and DRIVING flag updates...")
    ...
    def run(self, model_path=None):
        """Main execution function with dynamic AMI-based configuration, acceleration tracking, mission completion, and ROS2 publishing"""
        try:
            ...
            print("System ready! Racing with AMI state monitoring, driving flag control, acceleration tracking, mission completion publishing, and ROS2 publishing.")
            ...
            print("📡 NEW: Driving flag publishing enabled - Bool to /hydrakon_can/driving_flag")
            print("📡 NEW: AMI state monitoring enabled - dynamic configuration based on /hydrakon_can/state_str")
            print("📡 NEW: DRIVING state monitoring enabled - automatic driving flag control")
            ...
            print(f"   📡 Driving flag: /hydrakon_can/driving_flag (Bool)")
            ...
            print("🚗 System will automatically publish driving flag when DRIVING state changes!")
            if effective_target_laps:
    ...
    def cleanup(self):
        ...
        if self.controller and hasattr(self.controller, 'lap_counter'):
            ...
            print(f"\n{'='*60}")
            print(f"🏁 FINAL RACING STATISTICS WITH MISSION COMPLETION & ROS2")
            print(f"{'='*60}")
            ...
            if self.ros2_publisher:
                ...
                print(f"   Driving Flag: {driving_published}")
...
def main():
    # Create and run the racing system with dynamic AMI configuration
    racing_system = RacingSystem()
    
    try:
        success = racing_system.run()
        if success:
            print("Racing system with dynamic AMI configuration and ROS2 publishing completed successfully")
        else:
            print("Racing system with dynamic AMI configuration and ROS2 publishing failed to start")
